...
@interface LFPopOverView ()
@property (nonatomic, strong, readwrite) UIControl *blackOverlay;
@property (nonatomic, weak) UIView *containerView;
@property (nonatomic, assign, readwrite) LFPopoverPosition popoverPosition;
...
+ (instancetype)popover {
    return [[LFPopOverView alloc] init];
}
...
- (void)commonInit {
    ...
    self.sideEdge = 10.0;
    self.maskType = LFPopoverMaskTypeBlack;
...
- (void)showAtPoint:(CGPoint)point
    ...
    NSAssert(contentWidth > 0 && contentHeight > 0,
             @"LFPopover contentView bounds.size should not be zero");
    NSAssert(containerWidth > 0 && containerHeight > 0,
             @"LFPopover containerView bounds.size should not be zero");
    ...
    switch (self.maskType) {
        case LFPopoverMaskTypeBlack:
            maskColor = [UIColor colorWithWhite:0.0 alpha:0.3];
            break;
        case LFPopoverMaskTypeNone: {
...
- (void)showAtView:(UIView *)atView
   ...
   withContentView:(UIView *)contentView
    ...
    LFPopoverPosition popoverPosition;
    if (upCanContain) {
        popoverPosition = LFPopoverPositionUp;
        atPoint.y = CGRectGetMinY(atViewFrame) - betweenArrowAndAtView;
    } else {
        popoverPosition = LFPopoverPositionDown;
       ...
       if (upCanContain && downCanContain) {
        ...
        if (position != 0) {
            useUp = position == LFPopoverPositionUp ? YES : NO;
        }
        if (useUp) {
            popoverPosition = LFPopoverPositionUp;
            atPoint.y = CGRectGetMinY(atViewFrame) - betweenArrowAndAtView;
...
- (void)show {
    ...
    CGFloat originY = 0.0;
    if (self.popoverPosition == LFPopoverPositionDown) {
